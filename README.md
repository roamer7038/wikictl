...
| 5 | a git command failed. This includes reads: when git fails while `search`, `get`, `ls`, `lint`, `dirs` or `context` reads the wiki, or while `put`, `mv` or `rm` reads it before writing, the command exits with 5 and prints no partial result. A page that exists but cannot be read from the mirror exits with 5, not with `page not found` |
