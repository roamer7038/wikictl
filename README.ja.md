...
パスを省略できるコマンド（`grep`、`ls`、`find`、`lint`、`tree`）は、パスを省略すると wiki 全体を対象にします。wikictl がカレントディレクトリから対象のディレクトリを選ぶことはありません。
...
`grep -l` のように 1 行に 1 つずつ表示されるパスは、`| tr '\n' '\0' | xargs -0 -r wikictl ls -lt` の形で次のコマンドに渡します。引用符や空白を含む名前もそのまま渡り、`-r` は一致が 0 件のときに何も実行しません。改行を含む名前は、この形では扱えません。
