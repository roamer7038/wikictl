...
ミラーであることは、履歴の読み取りにも限界を与えます。`cat --at HEAD` と `cat --at <branch>` は解決しません。ミラーが持つ ref は tracking ref（`refs/remotes/origin/*`）とタグだけだからです。タグは fetch で更新も削除もされないため、リモートで付け替えられたタグは古い値のまま残ります。また、力押しの push で到達不能になったコミットは、ミラーの自動 gc（既定で 2 週間）で消えることがあり、そうなると `log` が表示した sha はもう解決できません。
