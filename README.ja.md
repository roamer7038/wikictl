...
- `put` には、フロントマターを含むページ全体を標準入力で渡す。不正なフロントマター、サイズの上限を超えるページ（`frontmatter_invalid`、`page_too_large`）、不正なパスは終了コード 4 で拒否する。それ以外の問題（`missing_summary`、`broken_link`、`links_syntax`、`name_style`）は警告を表示したうえでページを書き込む
...
| 4 | ページが wiki の形式に違反している。`put` は不正なフロントマター、サイズの上限を超えるページ、不正なパスを、`mv` は不正な移動先のパスを拒否する。`lint` は指摘が 1 件でもあれば 4 で終了する |
| 5 | git コマンドの失敗 |
