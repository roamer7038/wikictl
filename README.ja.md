...
| 5 | git コマンドの失敗。読み取りも含む。`search`、`get`、`ls`、`lint`、`dirs`、`context` が wiki を読み取るとき、または `put`、`mv`、`rm` が書き込みの前に読み取るときに git が失敗した場合は、不完全な結果を出力せずに 5 で終了する。存在するがミラーから読み取れないページも、`page not found` ではなく 5 で終了する |
