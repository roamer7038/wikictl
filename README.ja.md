...
wikictl はディレクトリの名前に意味を持たせません。ページはどのディレクトリにも置けます。パスを省略できるコマンド（`grep`、`ls`、`find`、`lint`、`tree`）は、パスを省略すると wiki 全体を対象にします。wikictl がカレントディレクトリから対象のディレクトリを選ぶことはありません。
