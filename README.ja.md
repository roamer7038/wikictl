...
- `summary` は推奨で、必須ではない。`summary` がないページでも `put` は `missing_summary` の警告を出して書き込み、`lint` は `missing_summary` を報告する。`search` と `ls` は代わりにタイトルを表示する。タイトルは最初の見出しの文字列で、見出しがなければファイル名とする。見出し末尾の `#` の並びは前に空白がある場合だけ除くため、`# C#` のタイトルは `C#` になる。フロントマター自体がないページも同じ扱い
...
- `## Links` 見出しは、ページの最後の見出しである場合だけ Links セクションの開始になる。後ろに別の見出しがあると、その行は本文として扱われて関係は無視され、`lint` はその見出しを `links_syntax` として報告する
...
- コードフェンスは CommonMark に従う。フェンスは、3 スペースまでのインデントの後に 3 個以上のバッククォートかチルダを並べた行で始まり（バッククォートの場合、行の残りにバッククォートを含まないこと）、同じ記号を開始行と同じ数以上並べ、後ろに空白だけがある行で閉じる。そのため、フェンス内の ```` ```bash ```` のような行ではフェンスは閉じない。閉じられないフェンスはページの末尾まで続く
...
`--no-fetch` を付けても、書き込み時にコミットの前に行う fetch は省略されません。`mv` と `rm` は fetch していないミラーの内容から変更を作るため、`--no-fetch` を付けると、読み取ったページが最後の fetch の後に変更されていた場合に衝突（終了コード 3）になります。その場合はコマンドを再実行してください。
...
- `mv` は、他のページから移動したページ（ディレクトリを移動する形式では、その配下のページ）へのリンクと、移動したページ内の相対リンクのうち移動によってリンク先が変わるものを、同じコミットで書き換える。それ以外のリンクは書かれたまま残すため、移動と無関係なページはコミットに含まれない。`rewritten` は、リンクを書き換えた他のページの数だけを数える。書き換えるリンクで変わるのはパスだけで、先頭の `./`、山括弧、クエリ、フラグメント、title は保つ。コードスパンとコードフェンスの中のリンクは書き換えない。書き換えたページは BOM を保ち、フロントマターと区切り行を含むすべての行の改行コードを、最初の行の改行コード（CRLF または LF）にそろえる。ファイル名が変わる場合は、元のファイル名（`.md` を除く）を `aliases` に追加する（既に含まれていれば追加しない）。追加するのは、ページのフロントマターが空かブロック形式の YAML のマッピングで、`aliases` がない、配列（ブロック形式またはフロー形式）、または null（`aliases:`、`aliases: ~`、`aliases: null`）の場合だけ。それ以外の場合（`aliases` が文字列の場合や、フロントマターがない場合など）は、警告を出さずに alias を追加しないまま移動する。移動先が既に存在する場合は終了コード 1 で失敗する。ディレクトリを移動する形式では、`<newdir>/` の配下にページが 1 つでもあれば失敗する
...
`mv` と `rm` には `--base` がなく、読み取った時点の各ページの blob sha を使います。変更または削除するページ（移動するページ、リンク元のページ、削除するページ）がコミットまでの間に変更・削除された場合や、移動先にページが作られた場合は、終了コード 3 で終了し、何も書き込みません。衝突は、該当する最初のページについて `put` と同じ形式で出力されます（[出力](#出力)を参照）。コマンドを再実行してください。衝突を報告した時点で新しい内容は fetch 済みです。
...
`put`、`mv`、`rm` が衝突を報告するときは、現在のページの内容も出力します。テキストでは、標準エラーに次の 1 行を、標準出力に現在の内容を出力します。
...
`<reason>` は、`--base` なしで書き込もうとしたページ（または `mv` の移動先）が既に存在する場合は `exists`、ページの sha が `--base` で指定した値（または `mv`、`rm` が読み取った値）と異なる場合は `changed` です。ページが削除されていた場合、`sha` と `content` は空になります。`message` は、ページが既に存在する、変更された、削除された、のどれに当たるかを説明します。`mv` と `rm` の場合は、コマンドを再実行するよう案内します。
...
| 5 | git コマンドの失敗。読み取りも含む。`search`、`get`、`ls`、`lint`、`dirs`、`context` が wiki を読み取るとき、または `init`、`put`、`mv`、`rm` が書き込みの前に読み取るときに git が失敗した場合は、不完全な結果を出力せずに 5 で終了する。存在するがミラーから読み取れないページも、`page not found` ではなく 5 で終了する |
...
| `links_syntax` | Links セクションの行がリンク行の形式になっていない、または `## Links` 見出しの後ろに別の見出しがある | 警告 |
