...
- 本文に「互換性への影響」節を設けます（英語の見出し「Compatibility impact」でもかまいません）。
...
- この文書（英語版と日本語版）を変える場合も、2 つの版の内容をそろえます。
...
- [Conventional Commits](https://www.conventionalcommits.org/ja/v1.0.0/) の形式にし、説明は英語か日本語で書きます（例: `fix: push の ref ロック失敗を再試行する`）。
