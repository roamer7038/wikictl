...
func TestJSONContract(t *testing.T) {
	...
	cases := []struct {
		...
		{"log", "", "", []string{"log", "global/push.md"}, ExitOK,
			[]string{"items", "items[].author", "items[].author_date", "items[].commit", "items[].commit_date",
				"items[].subject"}},
		{"log/follow", "", "", []string{"log", "--follow", "global/push.md"}, ExitOK,
			[]string{"items", "items[].author", "items[].author_date", "items[].commit", "items[].commit_date",
				"items[].paths", "items[].subject"}},
		{"links", "", "", []string{"links", "global/push.md"}, ExitOK,
			[]string{"items", "items[].direction", "items[].line", "items[].note", "items[].path", "items[].target",
				"items[].type", "items[].url"}},
