...
func TestGitFailure(t *testing.T) {
	...
	cases := []struct {
		...
		{"links/paths", gitFault{match: " ls-tree -r -z "}, "", []string{"links", "global/index.md"}, ExitGit},
		{"links/backlinks grep", gitFault{match: " grep -z --no-column -E -l "}, "", []string{"links", "global/index.md"}, ExitGit},
		{"links/cat", gitFault{match: " cat-file --batch "}, "", []string{"links", "global/index.md"}, ExitGit},
...
func TestUnreadableObject(t *testing.T) {
	...
	cases := []struct {
		...
		{"lint/blob under a directory", "global/push.md", "", "", []string{"--no-fetch", "lint", "global"}},
		{"lint/blob of the whole tree", "global/push.md", "", "", []string{"--no-fetch", "lint"}},
		{"ls/blob with all", "global/push.md", "", "", []string{"--no-fetch", "ls", "-a", "global"}},
		{"find/blob with meta", "global/push.md", "", "", []string{"--no-fetch", "find", "global", "-meta", "type=policy"}},
		{"lint/link target blob", "global/index.md", "", "", []string{"--no-fetch", "lint", "global/push.md"}},
		{"mv/source blob", "global/push.md", "", "", []string{"mv", "global/push.md", "global/push2.md"}},
		{"mv/other page blob", "global/push.md", "", "", []string{"mv", "projects/app/x.md", "projects/app/z.md"}},
		{"put/link target blob", "global/index.md", "", newPage + "[i](../../global/index.md)\n", []string{"put", "projects/app/new.md"}},
